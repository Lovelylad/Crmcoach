...
export const fetch = createAsyncThunk('businesses/fetch', async (data: any) => {
  const { id, query } = data;
  const result = await axios.get(`businesses${query || (id ? `/${id}` : '')}`);
  return id
    ? result.data
...
export const deleteItemsByIds = createAsyncThunk(
  'businesses/deleteByIds',
  async (data: any, { rejectWithValue }) => {
    try {
      await axios.post('businesses/deleteByIds', { data });
    } catch (error) {
      if (!error.response) {
        throw error;
      }
...
export const create = createAsyncThunk(
  ...
  async (data: any, { rejectWithValue }) => {
    try {
      const result = await axios.post('businesses', { data });
      return result.data;
    } catch (error) {
      if (!error.response) {
        throw error;
...
export const uploadCsv = createAsyncThunk(
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const result = await axios.post('businesses/bulk-import', data, {
        headers: {
...
export const update = createAsyncThunk(
  ...
  async (payload: any, { rejectWithValue }) => {
    try {
      const result = await axios.put(`businesses/${payload.id}`, {
        id: payload.id,
        data: payload.data,
      });
      return result.data;
...
export const { setRefetch } = businessesSlice.actions;
