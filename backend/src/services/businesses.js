...
module.exports = class BusinessesService {
  static async create(data, currentUser) {
    ...
    try {
      await BusinessesDBApi.create(data, {
        currentUser,
        transaction,
      });
  ...
  static async update(data, id, currentUser) {
    const transaction = await db.sequelize.transaction();
    try {
      let businesses = await BusinessesDBApi.findBy({ id }, { transaction });

      if (!businesses) {
        throw new ValidationError('businessesNotFound');
      }

      const updatedBusinesses = await BusinessesDBApi.update(id, data, {
        currentUser,
        transaction,
      });
  ...
  static async remove(id, currentUser) {
    ...
    try {
      await BusinessesDBApi.remove(id, {
        currentUser,
        transaction,
      });
